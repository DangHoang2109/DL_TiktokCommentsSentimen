...
# Process Array Comment in Dashboard production with kafka
def ProcessArrayComment(data_byte):
    ...
    fileName = f"file_{count_positive}.json"
    with open(fileName, 'w') as f:
        json.dump(comments_array, f, indent=4)
...
def load_random_json(folder_path):
    ...
    # Load and return the content of the random JSON file
    with open(random_file, 'r') as file:
        data = json.load(file)
        return data

folder_path = 'dataoffline'  # Update this to your folder path

while True:
    # Load one random JSON file
    data = load_random_json(folder_path)
    
    print(data[:2])

    temp_count_positive,temp_count_neutral,temp_count_negative,user_ids_negative,user_ids_neutral,user_ids_positive = Editor_TestOfflineData(data)
    print(temp_count_positive,temp_count_neutral,temp_count_negative)
...
# Create the line chart
    ...
    # Wait for 30 seconds before loading another file
    time.sleep(5)
